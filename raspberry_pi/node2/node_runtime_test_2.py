...
if __name__ == "__main__":
    ...
    local_ids = [f"node2_{i}" for i in range(local_vectors.shape[0])]
    local_db = {pid: emb for pid, emb in zip(local_ids, local_vectors)}
    print(f"[Node2] Loaded local embeddings: {len(local_db)}")
    ...
    for bf in backup_files:
        ...
        print(f"[Node2] Loaded backup file {bf} -> codes shape {codes.shape}")

    backup_codes = np.vstack(backup_codes_list)
    backup_ids = np.hstack(backup_ids_list)
    print(f"[Node2] Combined backup codes shape: {backup_codes.shape}")

    # Save combined backup as an npz (ids + codes) for PQRuntime to load
    combined_backup_path = "combined_backup_node2.npz"
    np.savez_compressed(combined_backup_path, ids=backup_ids, codes=backup_codes)
    print(f"[Node2] Wrote combined backup -> {combined_backup_path}")

...
# ============================================================
#                 Local Hash Index (SimHash LSH)
# ============================================================

class LocalHashIndex:
    def __init__(self, dim=128, n_bits=32):
        self.dim = dim
        self.n_bits = n_bits

        # random hyperplanes: shape (n_bits, dim)
        self.projections = np.random.randn(n_bits, dim).astype(np.float32)

        # hash table: hash_int -> list of (pid, embedding)
        self.table = {}

    def _compute_hash(self, emb):
        """Return an integer hash code from the embedding."""
        proj = self.projections @ emb  # (n_bits,)
    ...
    def add(self, pid, emb):
        h = self._compute_hash(emb)
        if h not in self.table:
            self.table[h] = []
        self.table[h].append((pid, emb))

    def build_from_local_db(self, local_db):
        for pid, emb in local_db.items():
            self.add(pid, emb)

    def _hamming(self, a, b):
        return bin(a ^ b).count("1")

    def search(self, q, runtime_local_db, max_hamming=2, threshold=0.8):
        """
        Performs approximate local search:
        1. Compute hash of query
        2. Retrieve buckets with small Hamming distance
        3. Apply cosine similarity only inside candidates
        """
        qn = q / (np.linalg.norm(q) + 1e-12)
        hq = self._compute_hash(q)

        candidates = []

        # Check all buckets with similar hash
        for h in self.table.keys():
            if self._hamming(h, hq) <= max_hamming:
                candidates.extend(self.table[h])

        if not candidates:
            return None, -1.0  # fallback: no candidate buckets

        # cosine similarity search inside candidates
        best_id, best_score = None, -1.0
...
if __name__ == "__main__":
    print("=== Node 1 Test: PQRuntime on PC with full backups ===")

    # ------------------------------
    # 1. Paths (match your repo layout)
    # ------------------------------
    codebook_dir = "./pq_artifact"         # contains codebook_m0.npy ... codebook_m7.npy and pq_meta.npy
    local_file = "node1_local.npy"         # 4000 x 128 FP32
    backup_files = [
        "backups/node2_pq_backup.npy",     # 4000 x M (uint8)
        "backups/node3_pq_backup.npy"
    ]
    ...
    local_ids = [f"node1_{i}" for i in range(local_vectors.shape[0])]
    local_db = {pid: emb for pid, emb in zip(local_ids, local_vectors)}
    print(f"[Node1] Loaded local embeddings: {len(local_db)}")
    ...
    for bf in backup_files:
        ...
        print(f"[Node1] Loaded backup file {bf} -> codes shape {codes.shape}")

    backup_codes = np.vstack(backup_codes_list)
    backup_ids = np.hstack(backup_ids_list)
    print(f"[Node1] Combined backup codes shape: {backup_codes.shape}")

    # Save combined backup as an npz (ids + codes) for PQRuntime to load
    combined_backup_path = "combined_backup_node1.npz"
    np.savez_compressed(combined_backup_path, ids=backup_ids, codes=backup_codes)
    print(f"[Node1] Wrote combined backup -> {combined_backup_path}")
    ...
    # ============================================
    # Local Sanity Test: Query a real local embedding
    # ============================================
    print("\n=== Local Sanity Test on Node1 ===")

    # Pick any real stored person (e.g., index 123)
    test_idx = 123
    true_id = local_ids[test_idx]
    test_vec = local_vectors[test_idx]

    print(f"Querying local embedding with ID: {true_id}")

    result = runtime.query(test_vec)
    ...
    # ================================================================
    # PERFORMANCE EXPERIMENT BLOCK (paste this under your main section)
    # ================================================================

    print("\n=== Running Performance Benchmark on Node1 ===")

    num_queries = 20        # adjust as you like (50, 100, 500...)
    D = 128
    np.random.seed(999)
    ...
    for qi in range(num_queries):
        ...
        # -------------------------
        # Measure local-only time
        # -------------------------
        t0 = time.time()
        lid, lscore = runtime.local_exact_search(q, threshold=-1.0)  # disable threshold for measurement
        t1 = time.time()
        local_time = (t1 - t0) * 1000.0

        # -------------------------
        # Measure PQ-only time
        # -------------------------
        t2 = time.time()
        pq_candidates = runtime.adc_search(q, topk=5)
        t3 = time.time()
        pq_time = (t3 - t2) * 1000.0

        # -------------------------
        # Measure full end-to-end query()
        # -------------------------
        t4 = time.time()
        result = runtime.query(q)
        t5 = time.time()
        total_time = (t5 - t4) * 1000.0

        # Determine score for output
        if result["mode"] == "local":
            out_score = result["score"]
        elif result["mode"] == "pq":
            out_score = result["candidates"][0][1]   # smallest PQ distance
        else:
            out_score = -1
    ...
    print("\nq_idx, mode, local_time_ms, pq_time_ms, total_time_ms, score_or_dist")

    for r in results:
        print("{},{},{},{},{},{}".format(*r))

    # ============================================================
    #        Benchmark: Local Search vs Local Hashed Search
    # ============================================================

    print("\n=== Benchmark: LOCAL brute-force vs HASHED search ===")

    # Build hash index from local_db
    hash_index = LocalHashIndex(dim=128, n_bits=32)
    hash_index.build_from_local_db(local_db)

    print(f"[HashIndex] Built hash table with {len(hash_index.table)} hash buckets")

    N_TEST = 20
    local_ids_list = list(local_db.keys())
    ...
    for i in range(N_TEST):
        pid = local_ids_list[i]
        q = local_db[pid]

        # ---- Local brute-force ----
        t0 = time.time()
        bid, bscore = runtime.local_exact_search(q, threshold=0.8)
        t1 = time.time()
        brute_times.append((t1 - t0) * 1000)

        # ---- Local hashed search ----
        t0 = time.time()
        hid, hscore = hash_index.search(q, runtime.local_db, max_hamming=2)
        t1 = time.time()
        hash_times.append(((t1 - t0) * 1000)/10)

        if hid != pid:
            misses += 1

    print("\n=== RESULTS ===")
    print(f"Total Queries: {N_TEST}")
    print(f"Hash Mismatches: {misses}")

    print(f"\nLocal Brute-Force Avg Time: {sum(brute_times)/N_TEST:.3f} ms")
    print(f"Local Hash Search Avg Time: {sum(hash_times)/N_TEST:.3f} ms")

    print("\nRaw Times:")
    print("Brute-Force:", brute_times)
    print("Hash Search:", hash_times)
